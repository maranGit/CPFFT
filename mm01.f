...
c
c           
c      subroutine mm01( span, felem, gpn, step, iter, ym_n1, nu_n1,
c     &                 beta, hprime_n1, lnelas, yld_n1, cgn, cgn1,
c     &                 deps, history, history1, rtse, dtemps,
c     &                 ym_n, nu_n )
...
c                       load step 1. 
c
c      call iodevn( local_in, local_out, trcmat, 2 )     
      local_debug = .false.
      if ( local_debug ) then
...
c     ****************************************************************
c
c      subroutine mm01_states_values( itype, elem_states_output,
c     &                                nrow_states, num_states  )
c
c                       access some global data structures
c
c      use elem_block_data, only: history_blocks, history_blk_list
c      use main_data, only: elems_to_blocks
c      
...
c                       parameters
c
c      integer :: nrow_states, itype, num_states
c      double precision :: elem_states_output(nrow_states,*)
c
c                       locals
c
c      double precision, 
c     & allocatable :: history_dump(:,:,:), one_elem_states(:)
c      integer :: relem, elnum, hist_size, blockno
c      logical :: do_a_block, local_debug
...
c         do_a_block = .false.
c         elnum = -itype
c         blockno = elems_to_blocks(elnum,1)
c      end if          
c
c      local_debug = .false.
c      felem       = elblks(1,blockno)
c      elem_type   = iprops(1,felem)
c      mat_type    = iprops(25,felem)
c      int_points  = iprops(6,felem)
c      span        = elblks(0,blockno)
c      hist_size   = history_blk_list(blockno)
      
c      if( local_debug ) write(out,9050) blockno, felem, elem_type,         
c     &         mat_type, int_points, span, hist_size
c
c           temporary block of history so it can be re-organized
c
c      allocate( one_elem_states(nrow_states) )
c      allocate( history_dump(hist_size,int_points,span) )
c      history_dump = reshape( history_blocks(blockno)%ptr,
c     &           (/hist_size,int_points,span/) )
c      
c      if( do_a_block ) then    
c        do relem = 1, span
c           elnum = felem + relem - 1  ! absolute element number
c           one_elem_states(1:nrow_states) = zero 
c           call mm01_states_values_a
c           elem_states_output(1:nrow_states,relem) = 
c     &                one_elem_states(1:nrow_states)
c        end do
c      else
c        relem = elnum + 1 - felem
c        one_elem_states(1:nrow_states) = zero
c        call mm01_states_values_a
c        elem_states_output(1:nrow_states,1) =
c     &                one_elem_states(1:nrow_states)
c      end if  
c        
c      deallocate( history_dump, one_elem_states )
c        
c      return
c      
c 9050 format(10x,"block, felem, etype, mtype:  ",4i7,
c     &  /,10x,   "int_pts, span, hist_size:    ",3i7 )
c 
c      contains
c     ========      
...
c      integer :: ipt   
c      double precision :: 
c     & epspls, kbar, state, back_stress(6), dword
c       integer :: iword(2)
c       equivalence ( dword, iword )
c
c      epspls =  zero
c      kbar = zero
c      state  = zero
c      back_stress(1:6) = zero
c       
c      do ipt = 1, int_points
c        kbar   = kbar + history_dump(2,ipt,relem)
c        epspls = epspls + history_dump(3,ipt,relem)
c        dword  = history_dump(4,ipt,relem)
c        state  = state + dble(iword(1))
c        back_stress(1:6) = back_stress(1:6) + 
c     &                     history_dump(6:11,ipt,relem)
c      end do
c
c      one_elem_states(1) = kbar / dble(int_points)            
c      one_elem_states(2) = epspls / dble(int_points)
c      one_elem_states(3) = state  / dble(int_points) 
c      one_elem_states(4:9) = back_stress(1:6) / dble(int_points)           
c
c      return     
c
c      end subroutine mm01_states_values_a
c      end subroutine mm01_states_values
c     ****************************************************************
c     *                                                              *
